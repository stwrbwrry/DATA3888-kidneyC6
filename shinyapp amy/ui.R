...
ui <- dashboardPage(
  ...
  dashboardHeader(title = "kidney gene expression dashboard", titleWidth = 400, tags$li(class = "dropdown", tags$style(".main-header {max-height: 400px}"))
  ),
  dashboardSidebar(
    sidebarMenu(
      menuItem("Tool", tabName = "0", icon = icon("laptop-code")),
      menuItem("Figure 1", tabName = "1", icon = icon("bezier-curve")),
            ...
            tabPanel("Dataset integration",
                     box(width=NULL, title = "Dataset combination",
                         ...
                         fluidRow(
                           ...
                           column(12, br()),
                           column(12, box(class = "descbox", width=NULL, collapsible = TRUE, collapsed = TRUE, title="View description", tags$b("Biological sex ", tags$a(href="https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3904696/", "model: ")), tags$p("While part of our motivation to extract biological sex data is to add an element to predictive model building, clinical researchers are becoming increasingly concerned in ensuring that models being built are fair when accounting for sex differences."), tags$p("We build the model by selecting three specific gene expression biomarkers. We used the pairwise differences of the logged expression levels of the following genes in the GSE46474 expression data, and then applied a KNN model to determine biological sex:" , tags$ul(tags$li("XIST -> higher relative expression levels for females"), tags$li("EIF1AY -> higher relative expression levels for males"), tags$li("RPS4Y1 -> neutral gene that can be used as a baseline"))))),
...
"), box(class = "descbox", width=NULL, title = "View description", collapsible = TRUE, collapsed = TRUE, tags$p("One of the aims of our dashboard is to give clinical researchers a way to combine their in-house data with public renal biopsy datasets. In doing so, this provides researchers with more data they can use to train their own models and analyze features."), tags$p("After combining datasets we have computed some fairness statistics to showcase that an example model built on the combined data will perform fairly on all of the combined datasets, as well as when considering biological sex. The tables below summarize fairness metrics when using a simple KNN model (k=7) on the combined dataset. This is an example of the analysis researchers may want to do when model building on the combined data.")))
      ...
      tabItem(
        tabName = "1", 
        fluidRow(box(width=12, title="Figure 1", img(src="figure1.png", width = "100%")))
      ),
      tabItem(
        tabName = "2", 
        fluidRow(
