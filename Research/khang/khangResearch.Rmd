...
little <- combinedDataset %>% select(-"outcome",-"source",-"gender") %>% as.matrix()


combinedDataset %>% select_if(negate(is.numeric))
khangTable <- boxplot_tbl(little, index =1)
box4_log = cbind(khangTable, source= combinedDataset$source)
# combinedDataset$samples <- rownames(combinedDataset)
