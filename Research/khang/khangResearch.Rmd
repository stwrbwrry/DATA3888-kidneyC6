...
z1 = exp_GSE36059 %>% as.matrix() # used for box plots
...
userFile <- data.frame(read_csv('../Liam/Data/userUploadData.csv', name_repair = "minimal"))
...
# dim(userExpression)
# length(userBinaryOutcomes)
ncol(userExpression)
ncol(userBinaryOutcomes)
ncol(z1)
ncol(y1)
ncol(z2)
ncol(y2)
...
# do we want to find name intersection

z0
# userFile <- read_csv('../Liam/Data/userUploadData.csv')
...
Variance <- as.data.frame(variance)
# class(userExpression)
# test <- as.data.frame(apply(userExpression,1,var))
userExpression = cbind(userExpression, variance = Variance)
userExpression = slice_max(userExpression, order_by = Variance, n = 2000) # For each column, get the 2000 most variable genes, so you get 2000 rows
userExpression = subset(userExpression, select = -c(Variance))
# 
# row_names_userExpression = rownames(userExpression) # used for finding common probe ids
