...
fData(bridgegse)$`Gene Symbol` = unlist(lapply(strsplit(fData(bridgegse)$`Gene Symbol`, " /// ", 1), `[`, 1))

idx = which(!duplicated(fData(bridgegse)$`Gene Symbol`) & !is.na(fData(bridgegse)$`Gene Symbol`))
...
# create a dataframe that only contains rows common in all dataframes
