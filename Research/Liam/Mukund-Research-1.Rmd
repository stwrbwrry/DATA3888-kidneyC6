...
GSE46474 <- readRDS("../khang/GSE46474.rds")
GSE36059 <-  readRDS("../khang/GSE36059.rds")
GSE48581 <-  readRDS("../khang/GSE48581.rds")
GSE129166 <-  readRDS("../khang/GSE129166.rds")
...
bb <- pairwise_col_diff(exp_GSE36059_log)
# colnames(bb)
...
box4_log = rbind(box1_log, box3_log)

colnames(box4_log)

logplot <-
ggplot(data = box1_log, aes(x = object, y = means)) +
  geom_point(aes(color = Dataset), size = 0.1) +
  geom_errorbar(aes(ymin = q1,
                    ymax = q3,
                    color = Dataset), size = 0.1,  alpha = 0.2) +
...
# network
names_uniq = unique(c(names$feature1, names$feature2)) # get the unique nodes
names_uniq
numbers = names
numbers
for (a in 1:nrow(numbers)) {
  for (i in 1:length(names_uniq)) {
    if (numbers$feature2[a] == names_uniq[i]) {
      numbers$feature2[a] = i # give feature 2 an id
    }
  }
}
...
for (a in 1:nrow(numbers)) {
  for (i in 1:length(names_uniq)) {
    if (numbers$feature1[a] == names_uniq[i]) {
      numbers$feature1[a] = i # give feature 1 id
    }
  }
}
numbers
...
head(dirty)
#dummy_col = dirty2$GSM3701704
dirty = dirty[c(-1)]
colnames = colnames(dirty)
colnames
## Assume that all data is numeric
...
for (j in c(1)){
  ...
  if (is.na(as.double(dummy_col[i])) == TRUE){
    ...
    dummy_col[i] < as.double(dummy_col[i])
  }
  }
  dirty[,j] = dummy_col
}
...
```{r}
userFile <- data.frame(read_csv('Data/dirtyuser.csv', name_repair = "minimal"))
...
Variance = rowVars(as.matrix(userFile), na.rm=TRUE)
Variance = as.data.frame(Variance)
userFile = as.data.frame(userFile)
userFile = cbind(userFile, variance = Variance)
userFile = slice_max(userFile, order_by = Variance, n = 2000)
userFile = subset(userFile, select = -c(Variance))
head(userFile)
userFile <- as.data.frame(t(userFile))
head(userFile)
userFile <- userFile %>% 
...
```{r}
a = as.factor(c(0.01,NA,0.03,0.04))
b = c(2,4,5,NA)
df1 = data.frame(a,b)
