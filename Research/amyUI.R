...
shinyUI(
  ...
  dashboardHeader(title = "Cross-platform omics prediction (CPOP) tool for kidney graft survival"),
  dashboardSideBar(
    sidebarMenu(
      menuItem("Tool", tabName = "0", icon = icon("square-code")),
      menuItem("Design", tabName = "1", icon = icon("bezier-curve")),
      menuItem("Data sources", tabName = "2", icon = icon("cube"))
      ),
    tags$footer(
      tags$p("By Mukund, Liam, Charlotte, Amy, Khang. Group: KidneyC6"), style="padding:0em 0em 0em 1em"
      )
    )
  ),
  dashboardBody(
    tabItems(
      ...
      tabItem(
        tabName = "1", 
        fluidRow(box(title="Model Design Diagram", img(src="figure1.png", align= "left")))),
      tabItem(
        tabName = "2", 
        fluidRow(
